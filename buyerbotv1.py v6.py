...
def report_categories_kb():
    # Reports can be uploaded even when Work is disabled.
    # Show every configured category so a disabled Work can still receive a report.
...
async def report_admin_action(m):
    ...
    if t=="Upload Report File":
        ...
        if not rows:
            return await ans(m, "❌ আগে Work/Category Management থেকে অন্তত একটি category তৈরি করুন।",reply_markup=am())
    ...
    if a=="upload_report_category":
        ...
        # Disabled Work is still valid for report uploads.
        rows=c.execute("SELECT * FROM categories ORDER BY id").fetchall()
...
async def withdrawal_toggle_callback(q):
    ...
    # Withdrawal status notice with the exact Premium Custom Emoji IDs supplied by the user.
    if new:
        notice=(
            "{{PE:4965219701572503640}} Withdrawal Notice..\n\n"
            "Admin এই মুহূর্তে উত্তোলন প্রক্রিয়া চালু করেছেন ।\n\n"
            "{{PE:6021739423975083484}} সময় থাকতে উত্তোলন রিকোয়েস্ট পাঠিয়ে দিন কারণ কিছু সময় পর আবার উত্তোলন প্রক্রিয়া বন্ধ করে দেয়া হবে। "
            "{{PE:6021739423975083484}}"
        )
    else:
        notice=(
            "{{PE:5420357600678004473}} Withdrawal Notice..\n\n"
            "Admin এই মুহূর্তে উত্তোলন প্রক্রিয়া বন্ধ করেছেন ।\n\n"
            "{{PE:6021739423975083484}} যদি উত্তোলন রিকোয়েস্ট করতে ব্যর্থ হয়ে থাকেন তাহলে নির্দিষ্ট সময় পর আবার উত্তোলন রিকোয়েস্ট করতে পারবেন।"
            "{{PE:6021739423975083484}}"
        )
    users=c.execute("SELECT user_id FROM users WHERE COALESCE(blocked,0)=0").fetchall()
    for u in users:
