...
BUTTON_EMOJI_IDS = {
    ...
    "admin_receiver_channel": "5425135356657815116",
    "admin_report_channel": "5203993413346680064",
...
async def report_admin_action(m):
    ...
    if t=="Upload Report File":
        cleanup_expired_report()
        # Report upload is completely independent of Work/Category status.
        # If categories exist, let the admin select one (including disabled
        # categories). If no category exists at all, accept the report anyway
        # and process any pending submissions globally.
        kb,rows=report_categories_kb()
        if not rows:
            state[ADMIN_ID]={"action":"upload_report","category_name":"","category_id":None}
            return await ans(m,
                "📊 Report Upload\n\n"
    ...
    if a=="upload_report_category":
        ...
        # Report uploads are intentionally independent of Work active/disabled status.
        # Always load every category here so an inactive/disabled Work can still receive a report.
        rows=c.execute("SELECT * FROM categories ORDER BY id").fetchall()
