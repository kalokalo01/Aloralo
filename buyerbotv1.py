...
async def process_report(bot, report_id, report_raw, category_name=""):
    """
    Process a report using the template's own OK-ID database. A submitted UID
    is green/OK exactly when it exists in the report's pre-existing Column-A
    master UID set, matching the report's COUNTIF duplicate rule.
    """
    ...
    # This is the exact result of pasting the user IDs into the report:
    ...
    for user_id, submissions in user_files.items():
        ...
        for s in submissions:
            for uid in submission_uids.get(s["id"],[]):
                ...
                if already:
                    continue
                # The report's own Total Payment multiplier is authoritative.
                # Fall back to the selected Work price if the report has no
                # usable multiplier.
        ...
        if paid_count:
            ...
            total_reward += reward
            results[user_id]=(paid_count,reward,matched_by_submission)

        # A submission is complete only when every UID in it has either been
        # paid or was already paid in an earlier report.
        for s in submissions:
            uids=submission_uids.get(s["id"],[])
            if uids and all(c.execute("SELECT 1 FROM paid_uids WHERE uid=?",(uid,)).fetchone() for uid in uids):
...
async def report_doc(m):
    ...
    # Remove only fully processed account uploads. Pending files with at least
    # one non-OK UID stay stored for the next report.
    ...
    # Tell each user exactly how many of their submitted UIDs were OK and how
    # much was credited. Users with zero matches do not receive a false payment.
    for uid_user,(ok_count,reward,matched_by_submission) in results.items():
        try:
            await sendmsg(
                m.bot,
                uid_user,
                f"📊 Report check completed.\n\n"
                f"🟢 আপনার OK UID: {ok_count}\n"
                f"💰 আপনার Balance-এ যোগ হয়েছে: {reward:.2f} BDT\n\n"
                f"আপনার OK account-এর payment সফলভাবে Balance-এ যোগ করা হয়েছে।",
                reply_markup=um(uid_user)
            )
        except Exception:
            pass

    # Users with pending submissions but zero newly-paid IDs should still know
    # the report was processed.
    if selected_category:
        pending_users=c.execute(
            "SELECT DISTINCT user_id FROM submissions "
            "WHERE status='pending' AND category_name=?",
            (selected_category,)
        ).fetchall()
        notified=set(results.keys())
        for row in pending_users:
            uid_user=row["user_id"]
            if uid_user in notified:
                continue
            try:
                await sendmsg(
                    m.bot,
                    uid_user,
                    f"📊 Report check completed.\n\n"
                    f"🟢 এই Report-এ আপনার নতুন OK UID পাওয়া যায়নি।\n"
                    f"💰 নতুন Balance যোগ হয়নি।",
                    reply_markup=um(uid_user)
                )
            except Exception:
                pass

    # General report notification for users who want to open the Report button.
