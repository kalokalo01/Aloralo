...
async def withdrawal_toggle_callback(q):
    ...
    notice=(f"{status_emoji} উত্তোলন প্রক্রিয়া আপডেট\n\n"
            f"Admin এই মুহূর্তে {x['name']} উত্তোলন প্রক্রিয়া {status_text} করেছে।\n"
            + ("এখন থেকে এই method দিয়ে withdrawal করা যাবে।" if new else "এই method দিয়ে বর্তমানে নতুন withdrawal করা যাবে না।"))
    users=c.execute("SELECT user_id FROM users WHERE COALESCE(blocked,0)=0").fetchall()
    for u in users:
        try:
