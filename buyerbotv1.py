...
def extract_report_green_uids(raw):
    """Return every UID whose report row/cell is marked as payable (green).

    The report workflow is: copy the submitted UIDs into column A, then the
    checker marks payable/OK accounts green.  We therefore inspect column A
    first and treat a UID as payable when its own cell is green.  For report
    formats that colour the whole row instead, row-level green detection is
    also supported.  Text statuses such as Green/Valid/Verified remain
    supported as a fallback.
    """
    green=set()
    wb=load_workbook(io.BytesIO(raw),read_only=False,data_only=True)
    for ws in wb.worksheets:
        # Per the bot's report format, UID is in column A.  We also honour an
        # explicit UID header if the sheet uses one.
        ...
        for row in ws.iter_rows():
            ...
            uid_cell=vals[uid_col-1]
            # Primary rule: the UID cell itself is green.
            payable=_cell_looks_green(uid_cell)

            # Some report generators colour the complete row rather than the
            # UID cell. Keep row-level detection as a compatibility fallback.
            if not payable:
                payable=row_is_green(vals)
        ...
        for s in submissions:
            ...
            for uid in uids:
                if uid in seen_user:
                    continue
                seen_user.add(uid)
                # Only UIDs that the uploaded report marked green are payable.
                # Matching is done against the exact normalized UID, independent
                # of row order or where that UID appeared in the user's file.
...
async def report_doc(m):
    ...
    await ans(m, 
        ...
        f"Total Reward Added: {total_reward:.2f} BDT\n"
        f"Green UID matching + balance update: completed\n"
