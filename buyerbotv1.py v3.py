...
async def withdrawal_toggle_callback(q):
    ...
    # Withdrawal Notice message updated per requested wording.
    # {{PE:...}} markers are converted to Telegram Premium Custom Emoji entities
    # by premiumize(), so the numeric IDs never appear as plain text to users.
    if new:
        notice=(
            "⭐ Withdrawal Notice..\n\n"
            "Admin এই মুহূর্তে উত্তোলন প্রক্রিয়া চালু করেছেন ।\n\n"
            "{{PE:4965219701572503640}} সময় থাকতে উত্তোলন রিকোয়েস্ট পাঠিয়ে দিন কারণ কিছু সময় পর আবার উত্তোলন প্রক্রিয়া বন্ধ করে দেয়া হবে। {{PE:6021739423975083484}}"
        )
    else:
        notice=(
            "⭐ Withdrawal Notice..\n\n"
            "Admin এই মুহূর্তে উত্তোলন প্রক্রিয়া বন্ধ করেছেন ।\n\n"
            "{{PE:5420357600678004473}} যদি উত্তোলন রিকোয়েস্ট করতে ব্যর্থ হয়ে থাকেন তাহলে নির্দিষ্ট সময় পর আবার উত্তোলন রিকোয়েস্ট করতে পারবেন। {{PE:6021739423975083484}}"
        )
    users=c.execute("SELECT user_id FROM users WHERE COALESCE(blocked,0)=0").fetchall()
    for u in users:
        try:
